...
const COMPANY = {
  ...
  mapQuery: "Công+ty+TNHH+Thực+Phẩm+Số+Một+B19+KP15+Tam+Hiệp+Đồng+Nai",
...
function buildVcf(data: PersonData) {
  const lines = [
    "BEGIN:VCARD",
    "VERSION:3.0",
    `FN:${data.name}`,
    `ORG:${COMPANY.vi}`,
    data.titleVi ? `TITLE:${data.titleVi}` : "",
    data.phone ? `TEL;TYPE=CELL:${data.phone}` : "",
    data.email ? `EMAIL:${data.email}` : "",
    `URL:${COMPANY.website}`,
    `ADR;TYPE=WORK:;;${COMPANY.addressVi};;;Việt Nam`,
    `NOTE:${COMPANY.taglineVi}`,
    "END:VCARD",
  ]
    .filter(Boolean)
    .join("\r\n");
...
function downloadVcf(data: PersonData) {
  const content = buildVcf(data);
  ...
  const slug = data.name
    ...
    .replace(/-+/g, "-")
    .replace(/^-|-$/g, "");
  a.download = `${slug || "lien-he"}.vcf`;
  a.href = url;
  a.click();
  URL.revokeObjectURL(url);
}

export function DynamicNamecard({ data }: { data: PersonData }) {
  const phoneRaw = data.phone.replace(/\s/g, "");
  const phoneDisplay = data.phone;
  const currentUrl = typeof window !== "undefined" ? window.location.href : COMPANY.website;
  ...
  const actions = [
    ...
    },
    {
      ...
      href: phoneRaw ? `https://zalo.me/${phoneRaw}` : undefined,
    },
    {
      id: "email",
      icon: Mail,
    ...
    },
    {
      ...
      color: "bg-white hover:bg-gray-50 text-[#133127] border border-[#0B8F3A]/30",
      href: `${COMPANY.website}/ho-so-nang-luc`,
    },
  ];
          ...
          <div className="mx-auto max-w-sm rounded-3xl bg-white p-6 shadow-xl shadow-[#0B8F3A]/10 ring-1 ring-[#0B8F3A]/10">
            ...
            <div className="text-center">
              <h1 className="text-2xl font-black tracking-wide text-[#133127]">
                {data.name || "THỰC PHẨM SỐ MỘT"}
              </h1>
              {data.titleVi && (
                <p className="mt-1 text-base font-semibold text-[#0B8F3A]">{data.titleVi}</p>
              )}
              {data.titleEn && (
                <p className="text-sm font-medium text-[#5e6d64]">{data.titleEn}</p>
              )}
              <div className="mt-4 rounded-xl bg-[#f0faf4] px-4 py-3">
                <p className="text-sm font-bold text-[#133127]">{COMPANY.vi}</p>
                <p className="mt-0.5 text-xs text-[#5e6d64]">{COMPANY.en}</p>
              </div>
              <div className="mt-4 border-t border-dashed border-[#0B8F3A]/20 pt-4">
                <p className="text-sm font-semibold italic text-[#0B8F3A]">"{COMPANY.taglineVi}"</p>
                <p className="mt-0.5 text-xs italic text-[#5e6d64]">"{COMPANY.taglineEn}"</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="px-4 pb-6">
        <div className="mx-auto max-w-sm space-y-3">
          <div className="grid grid-cols-2 gap-3">
            {actions.filter((a) => a.featured).map((btn) =>
              btn.onClick ? (
                <button
                  key={btn.id}
                  onClick={btn.onClick}
      ...
      <section className="px-4 pb-6">
        <div className="mx-auto max-w-sm rounded-2xl bg-white p-5 shadow-sm ring-1 ring-[#0B8F3A]/10">
          ...
          <ul className="space-y-3">
            {[
              phoneDisplay && { icon: Phone, text: phoneDisplay, href: `tel:${phoneRaw}` },
              data.email && { icon: Mail, text: data.email, href: `mailto:${data.email}` },
              { icon: Globe, text: COMPANY.website, href: COMPANY.website },
              { icon: MapPin, text: COMPANY.addressVi, href: `https://www.google.com/maps/search/?api=1&query=${COMPANY.mapQuery}` },
            ]
              .filter(Boolean)
              .map((item: any, i) => (
                <li key={i}>
                  ...
                  >
                    <span className="mt-0.5 flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-[#0B8F3A]/10">
                      <item.icon size={15} className="text-[#0B8F3A]" />
                    </span>
                    <span className="text-sm text-[#133127]">{item.text}</span>
                  </a>
                </li>
              ))}
          </ul>
          ...
          <div className="grid grid-cols-3 gap-3">
            {BRAND_VALUES.map((val) => (
              ...
              >
                ...
                <p className="mt-1.5 text-[10px] leading-tight text-[#5e6d64]">{val.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto max-w-sm text-center">
          <div className="mb-3 flex justify-center">
            <div className="relative h-8 w-28">
              <Image src="/images/tps1-logo-horizontal.png" alt="TPS1" fill className="object-contain" />
            </div>
          </div>
          <p className="text-xs font-semibold text-[#133127]">{COMPANY.vi}</p>
          <a href={COMPANY.website} className="mt-1 block text-xs text-[#0B8F3A] hover:underline">
            {COMPANY.website}
          </a>
