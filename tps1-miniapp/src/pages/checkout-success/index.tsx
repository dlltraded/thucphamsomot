...
export default function CheckoutSuccessPage() {
  ...
  const state = useLocation().state as CheckoutSuccessState | null;
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4 space-y-6">
        ...
        <div className="text-center space-y-3">
          ...
          <Text className="text-subtitle">
            Bước COD trên Zalo chỉ dùng để xác nhận gửi đơn, chưa ghi nhận khách hàng đã thanh toán.
          </Text>
          <Text className="text-subtitle">
            Đây là tổng tạm tính. Nhân viên sale sẽ kiểm tra thông tin khách hàng, chốt đơn giá cuối cùng và gửi phiếu xác nhận PDF trước khi giao hàng hoặc đối soát công nợ.
          </Text>
          {orderCode && (
            <div className="mx-auto mt-4 inline-flex rounded-full bg-primary/10 px-4 py-2 text-sm font-bold text-primary">
              Mã đơn: {orderCode}
            </div>
          )}
        </div>
