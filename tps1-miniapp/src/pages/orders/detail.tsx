...
export default function OrderDetailPage() {
  ...
  // Hóa đơn bán hàng — chỉ bấm được khi đơn đã "Hoàn thành" giao hàng, khác
  // với phiếu xác nhận ở trên (có ngay khi chốt giá). Đồng bộ với sale-webapp
  // (mục brief 2026-09-11).
  const handleOpenInvoicePdf = () => {
    if (!customer?.orderSessionToken || !order.centralOrderId) {
      toast.error("Phiên đăng nhập đã hết hạn. Vui lòng đăng nhập lại để tải hóa đơn.");
      return;
    }
    const url = `${CONFIG.API_BASE}/api/customer/order-invoice?orderId=${encodeURIComponent(order.centralOrderId)}&sessionToken=${encodeURIComponent(customer.orderSessionToken)}`;
  ...
  return <div className="min-h-full space-y-3 bg-background p-4 pb-8">
    ...
    {status === "completed" && order.invoiceDocumentId && customer?.orderSessionToken && order.centralOrderId && <Button fullWidth loading={openingPdf} disabled={openingPdf} onClick={handleOpenInvoicePdf}>Tải hóa đơn</Button>}
    {order.pricingStatus === "finalized" && order.confirmationDocumentId && customer?.orderSessionToken && order.centralOrderId && <Button fullWidth variant={status === "completed" ? "secondary" : "primary"} loading={openingPdf} disabled={openingPdf} onClick={handleOpenConfirmationPdf}>Tải PDF xác nhận đơn hàng</Button>}
