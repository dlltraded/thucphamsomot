...
export default function OrderDetailPage() {
  ...
  return <div className="min-h-full space-y-3 bg-background p-4 pb-8">
    ...
    {order.pricingStatus === "finalized" && order.confirmationDocumentId && customer?.orderSessionToken && order.centralOrderId && <Button fullWidth onClick={() => window.open(`${CONFIG.API_BASE}/api/customer/order-confirmation?orderId=${encodeURIComponent(order.centralOrderId!)}&sessionToken=${encodeURIComponent(customer.orderSessionToken)}`, "_blank")}>Tải PDF xác nhận đơn hàng</Button>}
  </div>;
}
