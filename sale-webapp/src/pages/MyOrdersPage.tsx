...
import { RefreshCw, ShoppingBag, MapPin, ChevronDown, ChevronUp } from 'lucide-react';
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {orders.map((order) => {
            ...
            return (
              <article key={order.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <button
                      ...
                      <tbody>
                        {(order.items || []).map((item: any) => (
                          <tr key={item.id} className="border-t border-slate-50">
                            ...
                            <td className="py-2 text-right text-slate-500">{item.quantity} {item.unit || ''}</td>
                            <td className="py-2 text-right text-slate-500">{money(item.finalUnitPrice ?? item.price)}</td>
                            <td className="py-2 text-right font-medium text-slate-700">{money(item.finalLineTotal ?? item.lineTotal)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                    {order.note && <p className="text-xs text-slate-400 mt-3">Ghi chú: {order.note}</p>}
                  </div>
                )}
              </article>
            );
