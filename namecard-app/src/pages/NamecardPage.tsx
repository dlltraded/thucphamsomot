...
import { fetchNamecard, type NamecardData } from '../lib/supabase'
...
function Skeleton() {
  return (
    <div style={{ background: BG, minHeight: '100vh', display: 'flex', justifyContent: 'center' }}>
      <div style={{ width: '100%', maxWidth: 430 }} className="animate-pulse">
        <div style={{ height: 90, background: '#ddd' }} />
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: -42, padding: '0 11px' }}>
          <div style={{ width: 96, height: 96, borderRadius: '50%', background: '#ccc', border: '4px solid #fff', marginBottom: 6 }} />
          <div style={{ height: 20, width: 200, background: '#ddd', borderRadius: 4, marginBottom: 5 }} />
          <div style={{ height: 20, width: 150, background: '#ccc', borderRadius: 99, marginBottom: 3 }} />
        </div>
      </div>
    </div>
  )
...
export default function NamecardPage() {
  ...
  useEffect(() => {
    setLoading(true)
    fetchNamecard(id || 'bach-nguyen').then(d => { setData(d); setLoading(false) })
  }, [id])

  useEffect(() => {
    if (data) QRCode.toDataURL(window.location.href, {
      width: 280, margin: 2, color: { dark: G, light: '#fff' }
  ...
  if (!data) return (
    <div style={{ background: BG, minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
      <div style={{ background: '#fff', borderRadius: 20, padding: 24, textAlign: 'center', maxWidth: 320, width: '100%' }}>
        <h1 style={{ fontSize: 17, fontWeight: 900, color: '#111', marginBottom: 8 }}>Không tìm thấy</h1>
        <p style={{ color: '#888', marginBottom: 16, fontSize: 13 }}>Danh thiếp không tồn tại.</p>
        <Link to="/bach-nguyen" style={{ background: G, color: '#fff', padding: '9px 22px', borderRadius: 12, textDecoration: 'none', fontWeight: 700, fontSize: 13 }}>Về trang chủ</Link>
      </div>
    </div>
  )
  ...
  const downloadQr = () => {
    if (!qrUrl) return
    const a = document.createElement('a'); a.href = qrUrl; a.download = `qr-${data.id}.png`; a.click()
  }
  ...
  const P = 11 // horizontal page padding in px
    ...
    <>
      ...
      <div className="nc">
        <div className="nw">
          ...
          {/* ── AVATAR + NAME ──────────────────────────────────── */}
          {/* Avatar center sits at wave = 90+24=114px from top; top half in banner, bottom half below */}
          <div className="f1" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginTop: -50, position: 'relative', zIndex: 2, paddingBottom: 0 }}>
            {/* Ring: 100px total */}
          ...
          {/* Avatar section total below wave: 50px(lower avatar) + 7 + 22 + 6 + 22 + 3 + 16 + 7 = 133px */}
          ...
          <div className="f2" style={{ padding: `0 ${P}px 5px`, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 7 }}>
            ...
            <a href={`https://zalo.me/${phoneRaw}`} target="_blank" rel="noopener noreferrer" className="tap"
              ...
              <div style={{ width: 46, height: 46, borderRadius: '50%', background: '#0068FF', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <button onClick={() => setShowQr(true)} className="tap"
              style={{ ...card, border: 'none', padding: '10px 4px 8px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 5 }}>
              <div style={{ width: 46, height: 46, borderRadius: '50%', background: OR, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
          ...
          {/* Contact total: ~176px (address row a bit taller) */}

          {/* ── COMPANY CARD ───────────────────────────────────── */}
          {/* Total: 10px+10px padding + max(68px logo, text block) ≈ 92px */}
          ...
          {/* ── SERVICE COMMITMENT ─────────────────────────────── */}
          {/* title: 18px + gap 5px + cards 58px = 81px */}
          <div className="f3" style={{ padding: `0 ${P}px 5px` }}>
          ...
          <footer className="f4" style={{ position: 'relative', background: 'linear-gradient(170deg, #D6F0E0 0%, #EEF7F2 40%, #C8EDD8 100%)', borderTop: '2px solid #A8D8B9', padding: '14px 16px 18px', textAlign: 'center', overflow: 'hidden', marginTop: 2 }}>
            ...
            {/* Animated leaf — bottom left */}
            <svg style={{ position: 'absolute', bottom: -8, left: -8, width: 90, height: 90, opacity: .22, animation: 'leafSway 4s ease-in-out infinite', transformOrigin: 'bottom left', pointerEvents: 'none' }} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
              ...
              <line x1="25" y1="70" x2="55" y2="48" stroke="#fff" strokeWidth="1" strokeOpacity=".3"/>
            </svg>

            {/* Animated leaf — bottom right */}
            <svg style={{ position: 'absolute', bottom: -10, right: -10, width: 85, height: 85, opacity: .2, animation: 'leafSway2 5s ease-in-out infinite', transformOrigin: 'bottom right', pointerEvents: 'none' }} viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
              ...
              <line x1="65" y1="55" x2="35" y2="35" stroke="#fff" strokeWidth="1" strokeOpacity=".25"/>
            </svg>

            {/* Small top-right accent leaf */}
            <svg style={{ position: 'absolute', top: 4, right: 18, width: 28, height: 28, opacity: .18, animation: 'leafSway 6s ease-in-out infinite reverse', pointerEvents: 'none' }} viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg">
              <path d="M5 45 C5 45 2 25 20 15 C30 9 43 10 45 5 C45 5 48 28 30 36 C20 41 5 45 5 45Z" fill="#009846"/>
            </svg>

            {/* Small top-left accent */}
            <svg style={{ position: 'absolute', top: 2, left: 14, width: 22, height: 22, opacity: .15, animation: 'leafSway2 7s ease-in-out infinite', pointerEvents: 'none' }} viewBox="0 0 50 50" xmlns="http://www.w3.org/2000/svg">
        ...
        <div onClick={e => e.target === e.currentTarget && setShowQr(false)}
          ...
          <div style={{ background: '#fff', borderRadius: 22, padding: '24px 18px 18px', width: '100%', maxWidth: 310, textAlign: 'center', position: 'relative', boxShadow: '0 18px 50px rgba(0,0,0,.2)' }}>
            ...
            <div style={{ width: 190, height: 190, margin: '0 auto 12px', borderRadius: 14, border: '2px solid #E5E7EB', padding: 8, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {qrUrl
                ? <img src={qrUrl} alt="QR" style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: 7 }} />
                : <div style={{ width: '100%', height: '100%', background: '#F3F4F6', borderRadius: 7 }} />}
            </div>
